...
def load_bi2014b_p300(data_dir: str, num_seen: int = 30, seed: int = 44) -> Dict:
    ...
    # Subject-level shuffle and split
    random.shuffle(all_files)
    seen_files = all_files[:num_seen]
    unseen_files = all_files[num_seen:]
    ...
    # Split trials for Seen Subjects
    for fname in seen_files:
        path = os.path.join(data_dir, fname)
        with np.load(path) as data:
            X, y = data['X'], data['y']
            
            # Shuffle trials within each subject
            ...
            X_train.append(X[:t_idx]); y_train.append(y[:t_idx])
            X_val.append(X[t_idx:v_idx]); y_val.append(y[t_idx:v_idx])
            X_test1.append(X[v_idx:]); y_test1.append(y[v_idx:])

    # Hold out Unseen Subjects entirely
    for fname in unseen_files:
        path = os.path.join(data_dir, fname)
        with np.load(path) as data:
            X_test2.append(data['X']); y_test2.append(data['y'])
